...
### 6.1 Implementation walkthrough (code-level)
- **Prescreener (`magnitude_prescreener.sv`)**  
  - Interface: `WIDTH`, `VEC_LEN`, `SUM_W`, `PROD_W` parameters. Inputs: flattened Q/K blocks, `threshold`, `valid_in`. Outputs: `valid_mask`, `product_mon`, `threshold_mon`.  
  - Pipeline: stage 1 computes absolute values and accumulates L1 sums (combinational). Stage 2 multiplies sums and compares to threshold. Latency = 2 cycles.  
  - Assertion hook: product monitor exported for SVA (no false negatives).  
  - TB (`tb/magnitude_prescreener_tb.sv`): random vectors, golden mask from software bound, latency-aware scoreboard.

- **Processing Element (`pe.sv`)**  
  - Inputs: `valid_in` to gate MAC, `load_acc` to seed accumulators. When `valid_in=0`, accumulator holds; when `load_acc=1`, accumulator is loaded (used at block start).  
  - Multiply-accumulate uses full `ACC_W` width; outputs propagate a/b downstream for systolic shift.

- **Systolic Array (`pe_array.sv`)**  
  - DIM×DIM grid; left/top edge inputs register each cycle; a_out/b_out shift right/down.  
  - `valid_mask` broadcast gates all PEs; `load_acc` seeds accumulators from `acc_init` when input valid aligns.  
  - TB (`tb/pe_array_tb.sv`): asserts hold when `valid_mask=0`; toggles valid to verify gating.

- **Tile (`tile_prescreen_array.sv`)**  
  - Instantiates prescreener + array; inserts two-stage shift registers on a_in/b_in to align with prescreener latency.  
  - Exposes `mask_out`, `valid_out`, and packed accumulator matrix.  
  - TB (`tb/tile_prescreen_array_tb.sv`): two scenarios—threshold set high (mask=0) → accum holds; threshold low (mask=1) → accum changes. Assertion monitors hold behavior.

- **Masked Softmax (`softmax_masked.sv`)**  
  - Pipeline: mask → max find → shift → LUT exp → sum → normalize.  
  - LUT: 4096-entry exp table (range [-8,8], step ~1/128) generated by `python/gen_exp_lut.py`. `LUT_ADDR=12`, `LUT_W=16` (Q0.16). Inputs assumed Q3.13; address uses arithmetic right-shift by FRAC=13 to approximate unit step.  
  - Normalization: integer divide `(exp_val << LUT_W)/exp_sum` (synthesizable; can replace with reciprocal LUT).  
  - TB (`tb/softmax_masked_tb.sv`): compares against behavioral softmax (real math) with tolerance; checks masked entry goes to zero.

- **AXI-Lite (`axi_lite_slave.sv`)**  
  - Registers: control/start, status(done/busy), threshold (low/high). Single-beat handshakes, always-ready channels for simplicity in sim.  
  - Threshold width matches `PROD_W` (52 bits).

- **Top (`top_dynasparse.sv`)**  
  - FSM states: IDLE→LOAD_Q→LOAD_K→PRESCREEN→COMPUTE→SOFTM→WRITEBACK. Currently stubs load paths; array driven via tile; softmax wiring left to complete (map accumulators to softmax inputs).  
  - AXI-Lite wired; buffers for Q/K noted as TODO for real data plane.

### 6.2 Latency alignment table
| Block | Latency (cycles) | Notes |
|-------|------------------|-------|
| Prescreener | 2 | abs+sum (comb), mult+cmp (reg) |
| Data aligner | 2 | shift registers on a_in/b_in in tile |
| PE array | 1 per MAC | accum updates gated by mask |
| Softmax | ~2 | LUT read + divide (current) |
Overall: mask is ready when data reaches PEs; softmax sits after accumulation.
...
- Regression: `vsim -c -do scripts/run_modelsims.do` compiles and executes all TBs headless.
